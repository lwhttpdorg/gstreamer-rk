...
/* For GST_VIDEO_FORMAT_PARAMETRIC the actual plane count is computed by
...
{
  if (G_UNLIKELY (info->finfo->format == GST_VIDEO_FORMAT_PARAMETRIC)) {
    gint n = 0, i;
    ...
    GstBuffer * buffer, gint id, GstMapFlags flags)
{
  ...
  /* For PARAMETRIC the parametric layout is fully described by info->stride[] /
   * info->offset[], so bypass the meta and use the direct buffer-mapping path.
   * */
  if (meta && info->finfo->format == GST_VIDEO_FORMAT_PARAMETRIC)
    meta = NULL;
    ...
    guint plane)
{
  ...
  /* For PARAMETRIC, plane validity is checked via strides. */
  g_return_val_if_fail (finfo->format == GST_VIDEO_FORMAT_PARAMETRIC
      || finfo->n_planes > plane, FALSE);

  sp = src->data[plane];
  dp = dest->data[plane];

  if (G_UNLIKELY (finfo->format == GST_VIDEO_FORMAT_PARAMETRIC)) {
    /* PARAMETRIC is a parametric format: finfo carries no component layout.
