...
typedef enum
{
  GENERIC_COLOR_RGB,
  GENERIC_COLOR_YCBCR,
  GENERIC_COLOR_GRAYSCALE,
  GENERIC_COLOR_UNKNOWN,
} GenericColorModel;

static GenericColorModel
detect_color_model (const GstGenericVideoParams * params)
{
  ...
  if (has_r && has_g && has_b)
    return GENERIC_COLOR_RGB;
  if (has_luma && has_cb && has_cr)
    return GENERIC_COLOR_YCBCR;
  if (has_gray || has_luma)
    return GENERIC_COLOR_GRAYSCALE;
  return GENERIC_COLOR_UNKNOWN;
}
...
static void
map_comp_to_channel (GstComponentType type, guint16 val16, guint16 chans[4])
{
  switch (type) {
    case GST_COMPONENT_TYPE_ALPHA:
      chans[0] = val16;
      break;
    case GST_COMPONENT_TYPE_RED:
    case GST_COMPONENT_TYPE_LUMA:
...
static guint16
channel_from_comp_type (GstComponentType type, const guint16 * pixel)
{
  switch (type) {
    case GST_COMPONENT_TYPE_ALPHA:
      return pixel[0];
...
static gboolean
validate_params_for_pack_unpack (const GstGenericVideoParams * params)
{
  guint i;
...
gst_video_format_generic_get_unpack_format (const GstVideoInfo * info)
{
  ...
  switch (cm) {
    case GENERIC_COLOR_RGB:
      return GST_VIDEO_FORMAT_ARGB64;
    case GENERIC_COLOR_YCBCR:
    case GENERIC_COLOR_GRAYSCALE:
...
/**
 * gst_video_format_generic_unpack: (skip)
 * @info: a #GstVideoInfo with format %GST_VIDEO_FORMAT_GENERIC
 ...
 * ARGB64 or AYUV64 (determined by gst_video_format_generic_get_unpack_format()).
 * Supports %GST_INTERLEAVE_TYPE_INTERLEAVE and %GST_INTERLEAVE_TYPE_COMPONENT,
 ...
 */
gboolean
gst_video_format_generic_unpack (const GstVideoInfo * info,
  ...
  switch (params->interleave_type) {
    case GST_INTERLEAVE_TYPE_INTERLEAVE:
    {
      ...
      for (px = 0; px < width; px++) {
        ...
        chans[0] = 0xffff;
        chans[1] = 0;
        chans[2] = (cm != GENERIC_COLOR_RGB) ? 0x8000 : 0;
        chans[3] = (cm != GENERIC_COLOR_RGB) ? 0x8000 : 0;
...
/**
 * gst_video_format_generic_pack: (skip)
 * @info: a #GstVideoInfo with format %GST_VIDEO_FORMAT_GENERIC
 * @flags: #GstVideoPackFlags
 ...
 */
gboolean
gst_video_format_generic_pack (const GstVideoInfo * info,
    GstVideoPackFlags flags,
    ...
    const gint stride[GST_VIDEO_MAX_PLANES], gint y, gint width)
{
  ...
  if (detect_color_model (params) == GENERIC_COLOR_UNKNOWN) {
