 ...
 *
 * Current payloads:
 * - #GstVideoParametricParams — parametric description for %GST_VIDEO_FORMAT_PARAMETRIC
 */
...
_ext_copy (GstVideoInfoExtensions * ext)
{
  ...
  if (ext->parametric_params)
    copy->parametric_params =
        g_memdup2 (ext->parametric_params, sizeof (GstVideoParametricParams));
...
gst_video_info_extensions_get_parametric_params_priv (const
    GstVideoInfoExtensions * ext)
{
  return ext->parametric_params;
}

void
gst_video_info_extensions_set_parametric_params_priv (GstVideoInfoExtensions *
    ext, const GstVideoParametricParams * params)
{
  g_free (ext->parametric_params);
  ext->parametric_params =
      params ? g_memdup2 (params, sizeof (GstVideoParametricParams)) : NULL;
}
