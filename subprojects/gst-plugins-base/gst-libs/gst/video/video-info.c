...
gst_video_info_from_caps (GstVideoInfo * info, const GstCaps * caps)
{
  ...
  if (gst_structure_has_name (structure, "video/x-raw")) {
    ...
    if (format == GST_VIDEO_FORMAT_PARAMETRIC) {
      GST_WARNING
          ("format=PARAMETRIC caps require gst_video_info_init_from_caps_extended()");
      return FALSE;
    }
...
gst_video_info_is_equal (const GstVideoInfo * info, const GstVideoInfo * other)
{
  ...
  if (GST_VIDEO_INFO_FORMAT (info) == GST_VIDEO_FORMAT_PARAMETRIC) {
    const GstVideoParametricParams *p_info =
        gst_video_info_extensions_get_parametric_params_priv
        (info->ABI.abi.extensions);
    const GstVideoParametricParams *p_other =
        gst_video_info_extensions_get_parametric_params_priv
    ...
    if (memcmp (p_info, p_other, sizeof (GstVideoParametricParams)) != 0)
      return FALSE;
  }
...
/**
 * gst_video_info_get_parametric_params:
 * @info: a #GstVideoInfo
 *
 * Returns the #GstVideoParametricParams associated with @info, or %NULL if
 * @info does not carry a %GST_VIDEO_FORMAT_PARAMETRIC format. The returned pointer
...
gst_video_info_get_parametric_params (const GstVideoInfo * info)
{
  ...
  return gst_video_info_extensions_get_parametric_params_priv
      (info->ABI.abi.extensions);
}
 ...
 * this function also accepts caps with format=PARAMETRIC.
 *
 * @info is always initialized before parsing, so it is safe to pass
 * uninitialized stack memory. If @info previously held PARAMETRIC extensions,
    ...
    const GstCaps * caps)
{
  ...
  GST_DEBUG ("parsing PARAMETRIC caps %" GST_PTR_FORMAT, caps);
  ...
  info->finfo = gst_video_format_get_info (GST_VIDEO_FORMAT_PARAMETRIC);
  info->width = width;
  info->height = height;
  ...
  /* PARAMETRIC-specific fields: component-definition, sampling-type,
   * interleave-type, stride/offset/size computation, and extension. */
  if (!gst_video_info_parametric_fill_info_priv (info, structure))
    return FALSE;
...
gst_video_info_to_caps (const GstVideoInfo * info)
{
  ...
  if (info->finfo->format == GST_VIDEO_FORMAT_PARAMETRIC) {
    const GstVideoParametricParams *params =
        gst_video_info_get_parametric_params (info);
    if (!params) {
      GST_ERROR ("PARAMETRIC format but no params attached");
      gst_caps_unref (caps);
      return NULL;
    }
    gst_video_info_parametric_fill_caps_priv (params, caps);
  }
