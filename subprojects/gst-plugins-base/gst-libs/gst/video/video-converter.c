...
#define FRAME_GET_R_LINE(frame,line) FRAME_GET_COMP_LINE(frame, GST_VIDEO_COMP_R, line)
#define FRAME_GET_G_LINE(frame,line) FRAME_GET_COMP_LINE(frame, GST_VIDEO_COMP_G, line)
#define FRAME_GET_B_LINE(frame,line) FRAME_GET_COMP_LINE(frame, GST_VIDEO_COMP_B, line)
...
static void
convert_RBG_32F_LE_RGB_task (FConvertTask * task)
{
  gint i;
  ...
  for (i = task->height_0; i < task->height_1; i++) {
    ...
    sr = FRAME_GET_R_LINE (task->src, i + task->in_y);
    sr += task->in_x;
    sg = FRAME_GET_G_LINE (task->src, i + task->in_y);
    sg += task->in_x;
    sb = FRAME_GET_B_LINE (task->src, i + task->in_y);
    sb += task->in_x;
    ...
    for (j = 0; j < task->width; j++) {
      d[j * 3 + 0] =
          (GFLOAT_FROM_LE (sr[j]) * task->r_scale) - task->r_offset;
      d[j * 3 + 1] =
          (GFLOAT_FROM_LE (sg[j]) * task->g_scale) - task->g_offset;
      d[j * 3 + 2] =
          (GFLOAT_FROM_LE (sb[j]) * task->b_scale) - task->b_offset;
    }
  }
}

static void
convert_RBG_32F_LE_RGB (GstVideoConverter * convert,
    const GstVideoFrame * src, GstVideoFrame * dest)
{
  int i;
  gint width = convert->in_width;
  ...
  gst_parallelized_task_runner_run (convert->conversion_runner,
      (GstParallelizedTaskFunc) convert_RBG_32F_LE_RGB_task,
      (gpointer) tasks_p);
}

static void
convert_RGB_RBG_32F_LE_task (FConvertTask * task)
{
  gint i;
  ...
  for (i = task->height_0; i < task->height_1; i++) {
    ...
    s = FRAME_GET_LINE (task->src, i + task->in_y);
    s += task->in_x * 3;
    dr = FRAME_GET_R_LINE (task->dest, i + task->out_y);
    dr += task->out_x;
    dg = FRAME_GET_G_LINE (task->dest, i + task->out_y);
    dg += task->out_x;
    db = FRAME_GET_B_LINE (task->dest, i + task->out_y);
    db += task->out_x;

    for (j = 0; j < task->width; j++) {
      dr[j] = GFLOAT_TO_LE ((s[j * 3 + 0] + task->r_offset) / task->r_scale);
      dg[j] = GFLOAT_TO_LE ((s[j * 3 + 1] + task->g_offset) / task->g_scale);
      db[j] = GFLOAT_TO_LE ((s[j * 3 + 2] + task->b_offset) / task->b_scale);
    }
  }
}

static void
convert_RGB_RBG_32F_LE (GstVideoConverter * convert,
    const GstVideoFrame * src, GstVideoFrame * dest)
{
  int i;
  gint width = convert->in_width;
  ...
  gst_parallelized_task_runner_run (convert->conversion_runner,
      (GstParallelizedTaskFunc) convert_RGB_RBG_32F_LE_task,
      (gpointer) tasks_p);
}

static void
convert_RBG_32F_BE_RGB_task (FConvertTask * task) /* TODO */
{
  gint i;
  ...
  for (i = task->height_0; i < task->height_1; i++) {
    ...
    for (j = 0; j < task->width; j++) {
      d[j * 3 + 0] = (GFLOAT_FROM_BE (sr[j]) * task->r_scale) - task->r_offset;
      d[j * 3 + 1] = (GFLOAT_FROM_BE (sg[j]) * task->g_scale) - task->g_offset;
      d[j * 3 + 2] = (GFLOAT_FROM_BE (sb[j]) * task->b_scale) - task->b_offset;
    }
  }
}

static void
convert_RBG_32F_BE_RGB (GstVideoConverter * convert,
    const GstVideoFrame * src, GstVideoFrame * dest)
{
  int i;
  gint width = convert->in_width;
  ...
  gst_parallelized_task_runner_run (convert->conversion_runner,
      (GstParallelizedTaskFunc) convert_RBG_32F_BE_RGB_task,
      (gpointer) tasks_p);
}

static void
convert_RGB_RBG_32F_BE_task (FConvertTask * task)
{
  gint i;
  ...
  for (i = task->height_0; i < task->height_1; i++) {
    ...
    for (j = 0; j < task->width; j++) {
      dr[j] = GFLOAT_TO_BE ((s[j * 3 + 0] + task->r_offset) / task->r_scale);
      dg[j] = GFLOAT_TO_BE ((s[j * 3 + 1] + task->g_offset) / task->g_scale);
      db[j] = GFLOAT_TO_BE ((s[j * 3 + 2] + task->b_offset) / task->b_scale);
    }
  }
}

static void
convert_RGB_RBG_32F_BE (GstVideoConverter * convert,
    const GstVideoFrame * src, GstVideoFrame * dest)
{
  int i;
  gint width = convert->in_width;
  ...
  gst_parallelized_task_runner_run (convert->conversion_runner,
      (GstParallelizedTaskFunc) convert_RGB_RBG_32F_BE_task,
      (gpointer) tasks_p);
}
...
get_scale_format (GstVideoFormat format, gint plane)
{
  ...
  switch (format) {
    ...
    case GST_VIDEO_FORMAT_RGB_32F_LE:
    case GST_VIDEO_FORMAT_RGB_32F_BE:
      res = format;
      break;
...
static const VideoTransform transforms[] = {
  ...
  {GST_VIDEO_FORMAT_RGB, GST_VIDEO_FORMAT_RGB_32F_LE, TRUE, FALSE, FALSE,
      FALSE, FALSE, FALSE, FALSE, FALSE, 0, 0, convert_RGB_RBG_32F_LE},
  {GST_VIDEO_FORMAT_RGB_32F_LE, GST_VIDEO_FORMAT_RGB, TRUE, FALSE, FALSE,
      FALSE, FALSE, FALSE, FALSE, FALSE, 0, 0, convert_RBG_32F_LE_RGB},

  {GST_VIDEO_FORMAT_RGB, GST_VIDEO_FORMAT_RGB_32F_BE, TRUE, FALSE, FALSE,
      FALSE, FALSE, FALSE, FALSE, FALSE, 0, 0, convert_RGB_RBG_32F_BE},
  {GST_VIDEO_FORMAT_RGB_32F_BE, GST_VIDEO_FORMAT_RGB, TRUE, FALSE, FALSE,
      FALSE, FALSE, FALSE, FALSE, FALSE, 0, 0, convert_RBG_32F_BE_RGB},
