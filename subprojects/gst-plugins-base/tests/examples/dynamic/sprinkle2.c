...
add_source (gdouble freq, gfloat pos)
{
  ...
  /* link src to fx and fx to audiomixer */
  gst_pad_link (info->fx_srcpad, info->audiomixer_sinkpad);
...
remove_source (SourceInfo * info)
{
   ...
   * reach the audiomixer sinkpad because the effect is in the NULL state. We will
   * send an EOS to audiomixer later. */
  gst_element_set_state (info->src, GST_STATE_NULL);

  /* unlink from audiomixer */
  gst_pad_unlink (info->src_srcpad, info->fx_sinkpad);
  gst_pad_unlink (info->fx_srcpad, info->audiomixer_sinkpad);
