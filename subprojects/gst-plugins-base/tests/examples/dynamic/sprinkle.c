...
add_source (gdouble freq)
{
  ...
  info->sinkpad = gst_element_request_pad_simple (audiomixer, "sink_%u");

  /* link pad to audiomixer */
...
remove_source (SourceInfo * info)
{
  ...
  /* give back the pad */
  gst_element_release_request_pad (audiomixer, info->sinkpad);
...
main (int argc, char *argv[])
{
  ...
  linked = gst_element_link_many (audiomixer, filter, convert, sink, NULL);
  g_assert (linked);
