...
class TsPlot(Analyzer):
        ...
        if entry_name == "new-element":
            ...
            if self.show_ghost_pads or pad_type not in ["GstGhostPad", "GstProxyPad"]:
                ...
                self.pad_info[ix] = "(%s, Pad Type: %s)" % (
                    self.element_info.get(parent_ix, ""),
                    pad_type,
                )
    ...
    def report(self):
        ...
        for ix, pad_file in self.buf_files.items():
            ...
            script += _PLOT_SCRIPT_BODY.substitute(
                self.params,
                title=name,
                subtitle=sub_title,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-g", "--ghost-pads", action="store_true", help="also plot data for ghost-pads"
    )
    parser.add_argument(
        "-s", "--size", action="store", default="1600x600", help="graph size as WxH"
    )
