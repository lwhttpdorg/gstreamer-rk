...
VALIDATE_TOOLS = [
    "gst-validate-1.0",
    "gst-validate-1.0.exe",
    "ges-launch-1.0",
    "ges-launch-1.0.exe",
    "gst-validate-transcoding-1.0",
    "gst-validate-transcoding-1.0.exe",
    "gst-validate-media-check-1.0",
    "gst-validate-media-check-1.0.exe",
]
...
class GstCheckTest(MesonTest):
    ...
    def get_subproc_env(self):
        ...
        cpath = set(env.get("GST_PLUGIN_PATH", "").split(os.pathsep)) | set(
            env.get("GST_PLUGIN_PATH_1_0", "").split(os.pathsep)
        )
...
class MesonTestsManager(TestsManager):
    ...
    def get_meson_tests(self):
        ...
        for i, bdir in enumerate(self.options.meson_build_dirs):
            ...
            output = subprocess.check_output([meson, "introspect", "--tests", bdir])
    ...
    def list_tests(self):
        ...
        for test in mesontests:
            if not self.setup_tests_from_sublauncher(test):
                self.add_test(
                    MesonTest(
                        self.get_test_name(test), self.options, self.reporter, test
                    )
                )
    ...
    def setup_tests_from_sublauncher(self, test):
        ...
        os.environ["GST_DEBUG_FILE"] = str(
            Path(self.options.logsdir) / f"{test['name']}.discover.gstlog"
        )
        res, _, tests_launcher = setup_launcher_from_args(
...
class GstCheckTestsManager(MesonTestsManager):
    ...
    def add_options(self, parser):
        ...
        arggroup.add_argument(
            "--gst-check-leak-trace-testnames",
            default=None,
        ...
        arggroup.add_argument(
            "--gst-check-leak-options", default=None, help="Leak tracer options"
        )
    ...
    def get_child_env(self, testname, check_name=None):
        ...
        if self.options.valgrind:
            child_env["CK_TIMEOUT_MULTIPLIER"] = str(
                VALGRIND_TIMEOUT_FACTOR * self.options.timeout_factor
            )
        ...
        if self.options.gst_check_leak_trace_testnames:
            if re.findall(self.options.gst_check_leak_trace_testnames, testname):
                ...
                tracers = set(os.environ.get("GST_TRACERS", "").split(";")) | set(
                    [leak_tracer]
                )
    ...
    def register_tests(self):
        ...
        if to_inspect:
            ...
            executor = conc.ThreadPoolExecutor(max_workers=self.options.num_jobs)
            tmp = []
        ...
        for test in mesontests:
            ...
            if os.path.basename(test["cmd"][0]) in [
                ...
                fpath = test["cmd"][1]
                self.add_test(
                    GstValidateSimpleTest(fpath, name, self.options, self.reporter)
                )
                continue
            if os.path.basename(test["cmd"][0]) in VALIDATE_TOOLS:
                ...
                self.add_test(
                    GstValidateCheckTest(
                        name, self.options, self.reporter, test, child_env
                    )
                )
            elif not gst_tests:
                child_env = self.get_child_env(name)
                self.add_test(
                    GstCheckTest(name, self.options, self.reporter, test, child_env)
                )
            else:
