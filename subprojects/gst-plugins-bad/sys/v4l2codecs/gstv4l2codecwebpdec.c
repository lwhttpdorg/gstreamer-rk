 ...
 * Copyright (C) 2024 Hugues Fruchet <hugues.fruchet@foss.st.com>
 *
...
GST_DEBUG_CATEGORY_STATIC (v4l2_webpdec_debug);
#define GST_CAT_DEFAULT v4l2_webpdec_debug
...
G_DEFINE_ABSTRACT_TYPE (GstV4l2CodecWebpDec, gst_v4l2_codec_webp_dec,
    GST_TYPE_VP8_DECODER);

#define parent_class gst_v4l2_codec_webp_dec_parent_class

static guint
gst_v4l2_codec_webp_dec_get_preferred_output_delay (GstVp8Decoder * decoder,
    gboolean is_live)
{

  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (decoder);
  guint delay;
...
static gboolean
gst_v4l2_codec_webp_dec_open (GstVideoDecoder * decoder)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (decoder);
  guint version;
...
static gboolean
gst_v4l2_codec_webp_dec_close (GstVideoDecoder * decoder)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (decoder);
...
static void
gst_v4l2_codec_webp_dec_streamoff (GstV4l2CodecWebpDec * self)
{
  if (self->streaming) {
...
static void
gst_v4l2_codec_webp_dec_reset_allocation (GstV4l2CodecWebpDec * self)
{
...
static gboolean
gst_v4l2_codec_webp_dec_stop (GstVideoDecoder * decoder)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (decoder);

  gst_v4l2_codec_webp_dec_streamoff (self);
  gst_v4l2_codec_webp_dec_reset_allocation (self);
...
static gboolean
gst_v4l2_codec_webp_dec_negotiate (GstVideoDecoder * decoder)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (decoder);
  GstVp8Decoder *webpdec = GST_VP8_DECODER (decoder);
  /* *INDENT-OFF* */
  ...
  if (!gst_v4l2_decoder_set_sink_fmt (self->decoder, V4L2_PIX_FMT_WEBP_FRAME,
          self->width, self->height, 12 /* 8 bits 4:2:0 */ )) {
    GST_ELEMENT_ERROR (self, CORE, NEGOTIATION,
        ("Failed to configure WebP decoder"),
  ...
  self->output_state =
      ...
      self->height, webpdec->input_state);
...
static gboolean
gst_v4l2_codec_webp_dec_decide_allocation (GstVideoDecoder * decoder,
    GstQuery * query)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (decoder);
  guint min = 0;
...
static void
gst_v4l2_codec_webp_dec_fill_segment (struct v4l2_vp8_segment
...
static void
gst_v4l2_codec_webp_dec_fill_lf (struct v4l2_vp8_loop_filter
...
static void
gst_v4l2_codec_webp_dec_fill_entropy (struct v4l2_vp8_entropy
...
static void
gst_v4l2_codec_webp_dec_fill_frame_header (GstV4l2CodecWebpDec * self,
    const GstVp8FrameHdr * frame_hdr)
{
  ...
  gst_v4l2_codec_webp_dec_fill_entropy (&self->frame_header.entropy, frame_hdr);
}

static void
gst_v4l2_codec_webp_dec_fill_references (GstV4l2CodecWebpDec * self)
{
  GstVp8Decoder *decoder = &self->parent;

  if (decoder->last_picture)
    self->frame_header.last_frame_ts =
        decoder->last_picture->subprojects/gst-plugins-bad/system_frame_number * G_GUINT64_CONSTANT (1000);

  if (decoder->golden_ref_picture)
    self->frame_header.golden_frame_ts =
        decoder->golden_ref_picture->subprojects/gst-plugins-bad/system_frame_number *
        G_GUINT64_CONSTANT (1000);

  if (decoder->alt_ref_picture)
    self->frame_header.alt_frame_ts =
        decoder->alt_ref_picture->subprojects/gst-plugins-bad/system_frame_number *
        G_GUINT64_CONSTANT (1000);
...
gst_v4l2_codec_webp_dec_new_sequence (GstVp8Decoder * decoder,
    const GstVp8FrameHdr * frame_hdr, gint max_dpb_size)
{
  ...
  gst_v4l2_codec_webp_dec_fill_frame_header (self, frame_hdr);
...
static GstFlowReturn
gst_v4l2_codec_webp_dec_start_picture (GstVp8Decoder * decoder,
  ...
  if (!self->bitstream) {
    ...
    if (!self->bitstream) {
      GST_ELEMENT_ERROR (decoder, RESOURCE, NO_SPACE_LEFT,
          ("Not enough memory to decode WebP stream."), (NULL));
...
static GstFlowReturn
gst_v4l2_codec_webp_dec_decode_picture (GstVp8Decoder * decoder,
    GstVp8Picture * picture, GstVp8Parser * parser)
{
  ...
  gst_v4l2_codec_webp_dec_fill_frame_header (self, &picture->frame_hdr);
  gst_v4l2_codec_webp_dec_fill_segment (&self->frame_header.segment,
      &parser->segmentation);
  gst_v4l2_codec_webp_dec_fill_lf (&self->frame_header.lf,
      &parser->mb_lf_adjust);
  gst_v4l2_codec_webp_dec_fill_references (self);
...
static void
gst_v4l2_codec_webp_dec_reset_picture (GstV4l2CodecWebpDec * self)
{
...
static GstFlowReturn
gst_v4l2_codec_webp_dec_end_picture (GstVp8Decoder * decoder,
    GstVp8Picture * picture)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (decoder);
  ...
  frame = gst_video_decoder_get_frame (GST_VIDEO_DECODER (self),
      picture->subprojects/gst-plugins-bad/system_frame_number);
  ...
  request = gst_v4l2_decoder_alloc_request (self->decoder,
      picture->subprojects/gst-plugins-bad/system_frame_number, self->bitstream, buffer);
  if (!request) {
    GST_ELEMENT_ERROR (decoder, RESOURCE, NO_SPACE_LEFT,
  ...
  gst_v4l2_codec_webp_dec_reset_picture (self);
...
static gboolean
gst_v4l2_codec_webp_dec_copy_output_buffer (GstV4l2CodecWebpDec * self,
...
static GstFlowReturn
gst_v4l2_codec_webp_dec_output_picture (GstVp8Decoder * decoder,
    GstVideoCodecFrame * frame, GstVp8Picture * picture)
{
  ...
  GST_DEBUG_OBJECT (self, "Output picture %u", picture->subprojects/gst-plugins-bad/system_frame_number);
  ...
  if (gst_v4l2_request_failed (request)) {
    GST_ELEMENT_ERROR (self, STREAM, DECODE,
        ("Failed to decode frame %u", picture->subprojects/gst-plugins-bad/system_frame_number), (NULL));
    goto error;
  }
  ...
  if (self->copy_frames)
    gst_v4l2_codec_webp_dec_copy_output_buffer (self, frame);
...
static void
gst_v4l2_codec_webp_dec_set_flushing (GstV4l2CodecWebpDec * self,
    gboolean flushing)
{
  if (self->sink_allocator)
...
static gboolean
gst_v4l2_codec_webp_dec_flush (GstVideoDecoder * decoder)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (decoder);
  ...
  gst_v4l2_codec_webp_dec_set_flushing (self, FALSE);

  return GST_VIDEO_DECODER_CLASS (parent_class)->flush (decoder);
}

static gboolean
gst_v4l2_codec_webp_dec_sink_event (GstVideoDecoder * decoder, GstEvent * event)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (decoder);
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_FLUSH_START:
      ...
      gst_v4l2_codec_webp_dec_set_flushing (self, TRUE);
      break;
    default:
      break;
  }
...
gst_v4l2_codec_webp_dec_change_state (GstElement * element,
    GstStateChange transition)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (element);
...
static void
gst_v4l2_codec_webp_dec_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (object);
...
static void
gst_v4l2_codec_webp_dec_get_property (GObject * object, guint prop_id,
...
static void
gst_v4l2_codec_webp_dec_init (GstV4l2CodecWebpDec * self)
{
  self->saw_header = FALSE;
}

static void
gst_v4l2_codec_webp_dec_subinit (GstV4l2CodecWebpDec * self,
    GstV4l2CodecWebpDecClass * klass)
{
...
static void
gst_v4l2_codec_webp_dec_dispose (GObject * object)
{
  GstV4l2CodecWebpDec *self = GST_V4L2_CODEC_WEBP_DEC (object);
...
static void
gst_v4l2_codec_webp_dec_class_init (GstV4l2CodecWebpDecClass * klass)
{
}

static gboolean
gst_v4l2_codec_webp_dec_set_format (GstVideoDecoder * decoder,
 ...
 */
static GstFlowReturn
gst_v4l2_codec_webp_dec_parse (GstVideoDecoder * decoder,
    GstVideoCodecFrame * frame, GstAdapter * adapter, gboolean at_eos)
{
  ...
  GstV4l2CodecWebpDec *self = (GstV4l2CodecWebpDec *) decoder;
...
static void
gst_v4l2_codec_webp_dec_subclass_init (GstV4l2CodecWebpDecClass * klass,
    GstV4l2CodecDevice * device)
{
  ...
  gobject_class->set_property = gst_v4l2_codec_webp_dec_set_property;
  gobject_class->get_property = gst_v4l2_codec_webp_dec_get_property;
  gobject_class->dispose = gst_v4l2_codec_webp_dec_dispose;

  gst_element_class_set_static_metadata (element_class,
      "V4L2 Stateless WebP Video Decoder",
      "Codec/Decoder/Video/Hardware",
      "A V4L2 based WebP video decoder",
      "Nicolas Dufresne <nicolas.dufresne@collabora.com>, Hugues Fruchet <hugues.fruchet@foss.st.com>");

  gst_element_class_add_static_pad_template (element_class, &sink_template);
  gst_element_class_add_static_pad_template (element_class, &src_template);
  element_class->change_state =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_change_state);

  decoder_class->open = GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_open);
  decoder_class->close = GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_close);
  decoder_class->stop = GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_stop);
  decoder_class->parse = GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_parse);
  decoder_class->set_format =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_set_format);
  decoder_class->negotiate =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_negotiate);
  decoder_class->decide_allocation =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_decide_allocation);
  decoder_class->flush = GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_flush);
  decoder_class->sink_event =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_sink_event);

  vp8decoder_class->new_sequence =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_new_sequence);
  vp8decoder_class->start_picture =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_start_picture);
  vp8decoder_class->decode_picture =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_decode_picture);
  vp8decoder_class->end_picture =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_end_picture);
  vp8decoder_class->output_picture =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_output_picture);
  vp8decoder_class->get_preferred_output_delay =
      GST_DEBUG_FUNCPTR (gst_v4l2_codec_webp_dec_get_preferred_output_delay);
...
void
gst_v4l2_codec_webp_dec_register (GstPlugin * plugin, GstV4l2Decoder * decoder,
    GstV4l2CodecDevice * device, guint rank)
{
  gchar *element_name;
  GstCaps *src_caps;

  GST_DEBUG_CATEGORY_INIT (v4l2_webpdec_debug, "v4l2codecs-webpdec", 0,
      "V4L2 stateless WebP decoder");

  if (!gst_v4l2_decoder_set_sink_fmt (decoder, V4L2_PIX_FMT_WEBP_FRAME,
          320, 240, 8))
    return;
  src_caps = gst_v4l2_decoder_enum_src_formats (decoder);

  if (gst_caps_is_empty (src_caps)) {
    GST_WARNING ("Not registering WebP decoder since it produces no "
        "supported format");
    goto done;
  }

  gst_v4l2_decoder_register (plugin, GST_TYPE_V4L2_CODEC_WEBP_DEC,
      (GClassInitFunc) gst_v4l2_codec_webp_dec_subclass_init,
      gst_mini_object_ref (GST_MINI_OBJECT (device)),
      (GInstanceInitFunc) gst_v4l2_codec_webp_dec_subinit,
      "v4l2sl%swebpdec", device, rank, &element_name);
