...
_process_nalu (GstRtpH266Pay * rtph266pay, GstBuffer * nalu_buf)
{
  ...
  if (g_queue_get_length (nalus) >= MAX_QUEUED_NALUS) {
    GST_WARNING_OBJECT (rtph266pay,
        "Too many NALUs enqueued, dropping oldest one.");
    _nalu_free (g_queue_pop_head (nalus));
  }

  _update_ps_cache (rtph266pay, nalu);
  g_queue_push_tail (nalus, nalu);
}
