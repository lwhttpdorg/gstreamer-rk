...
static GstBuffer *
gst_flv_mux_create_object_script_end_marker ()
{
  GstBuffer *tmp;
  guint8 *data;
...
gst_flv_mux_create_metadata (GstFlvMux * mux)
{
  ...
  for (GList * p = mux->audio_pads; p; p = p->next) {
    ...
    g_snprintf ((gchar *) id, 4, "%d", pad->track_id);

    tmp = gst_flv_mux_create_object_script_start_marker ((gchar *) id);
    script_tag = gst_buffer_append (script_tag, tmp);
  ...
  // go with legacy flv audio track pad values as default configurations
