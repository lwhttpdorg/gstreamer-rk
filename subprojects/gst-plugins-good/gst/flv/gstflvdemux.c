...
static void
_send_new_segment (GstFlvDemux * demux, gboolean new_segment, GstPad * pad)
{
  if (new_segment) {
    for (guint i = 0; i < demux->audio_tracks->len; i++) {
      GstFlvDemuxTrack *track = g_ptr_array_index (demux->audio_tracks, i);
    ...
    gst_pad_push_event (pad, gst_event_ref (demux->new_seg_event));
  } else {
    GST_WARNING_OBJECT (demux, "invalid pad to send the segment event");
  }
}
...
gst_flv_demux_parse_tag_audio (GstFlvDemux * demux, GstBuffer * buffer)
{
  ...
  guint32 tag_header_len = 0, pts_ext = 0;
  guint8 flags = 0;
  GstMapInfo map;
  GstBuffer *outbuf;
  guint8 *data;
  ...
  if (!gst_byte_reader_get_uint8 (&reader, (guint8 *) & pts_ext)) {
  ...
  gst_byte_reader_skip (&reader,
      tag_header_begin - gst_byte_reader_get_pos (&reader));
  ...
  if (map.size == MESSAGE_HEADER_LEN)
    goto beach;
  ...
  if (codec_tag == EXTENDED_AUDIO_HEADER) {
    ...
    while (audio_packet_type == MODEX) {
      ...
      if (!gst_byte_reader_get_uint8 (&reader, (guint8 *) & mod_ex_data_size)) {
        GST_ERROR_OBJECT (demux, "failed to parse modex data size");
        goto beach;
      };

      if (mod_ex_data_size == 255) {
        if (!gst_byte_reader_get_uint16_be (&reader,
                (guint16 *) & mod_ex_data_size)) {
          GST_ERROR_OBJECT (demux, "failed to parse modex data size");
          goto beach;
        };
      }

      GST_INFO_OBJECT (demux, "got a MODEX packet but not handling it");
    ...
    if (audio_packet_type == MULTITRACK) {
      ...
      /* chop off the higher byte in the track id.
       * it is a int16 just to have id as -1 for the legacy track
       */
      track_id &= 0xff;

      if (!gst_byte_reader_get_uint8 (&reader, (guint8 *) & track_id)) {
      ...
      offset = gst_byte_reader_get_pos (&reader);
      if (offset > tag_header_begin)
        tag_header_len = offset - tag_header_begin;
      else {
        GST_ERROR_OBJECT (demux, "position < tag_header_begin");
        goto beach;
      }

      if (tag_header_len !=
          EXHEADER_PLUS_PACKETYPE_LEN + MUTLITRACKTYPE_PLUS_PACKETYPE_LEN +
          FOURCC_LEN + TRACK_ID_LEN) {
        GST_DEBUG_OBJECT (demux, "size mismatch tag header len %u",
            tag_header_len);
      }
    } else {
      if (!_get_codec_tag (&reader, &codec_tag)) {
    ...
    if (codec_tag == NELLYMOSER_8K || codec_tag == MP3_8K
        || codec_tag == G711_ALAW || codec_tag == G711_MULAW)
      rate = 8000;
    else if ((codec_tag == NELLYMOSER_16K) || (codec_tag == SPEEX))
      rate = 16000;
    ...
    switch (aac_packet_type) {
      case 0:                  /* AAC sequence header */
      {
        ...
        track->codec_data =
            ...
            offset, demux->tag_data_size - tag_header_len);
  ...
  if (G_LIKELY (!track->pad)) {
    ...
    if (track_id >= 0) {
      g_snprintf (pad_name, 10, "audio_%u", track_id);
      templ_name = "audio_%u";
    } else {
      g_snprintf (pad_name, 10, "audio");
      templ_name = "audio";
    }
  ...
  if (G_UNLIKELY (track->need_segment)) {
    ...
    _send_new_segment (demux, replaced_prev_segment, track->pad);
