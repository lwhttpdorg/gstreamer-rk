    ...
    GstPadDirection direction, GstCaps * caps, GstCaps * othercaps)
{
  ...
  result =
      gst_video_convertscale_fixate_caps (GST_OBJECT (trans), direction,
      GST_VIDEO_ORIENTATION_IDENTITY, NULL, TRUE, TRUE, caps, othercaps);
  if (!result)
    return othercaps;

  gst_clear_caps (&othercaps);

  GST_DEBUG_OBJECT (trans, "fixated othercaps to %" GST_PTR_FORMAT, result);
