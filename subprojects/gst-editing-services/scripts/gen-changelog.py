...
def print_help():
    ...
    print(
        "gen-changelog: generate GNU-style changelog from git history", file=sys.stderr
    )
    print("", file=sys.stderr)
    print(
        "Usage: {} [OPTIONS] GSTREAMER-MODULE [START-TAG] [HEAD-TAG]".format(
            sys.argv[0]
        ),
        file=sys.stderr,
    )
...
def output_commits(module, start_tag, end_tag, subtree_path=None):
    ...
    p = subprocess.Popen(
        args=cmd, shell=False, stdout=subprocess.PIPE, cwd=meson_source_root
    )
    buf = []
    files = []
    filemode = False
...
def get_commit_date_for_ref(ref):
    ...
    r = subprocess.run(
        cmd, capture_output=True, text=True, check=True, cwd=meson_source_root
    )
...
def populate_release_tags_for_premonorepo_module(module_tag_prefix):
    ...
    for line in [x.decode("utf8") for x in p.stdout.readlines()]:
        git_tag = line.strip()
        version_str = (
            git_tag.removeprefix(module_tag_prefix)
...
def populate_release_tags_for_monorepo_subproject():
    ...
    for line in [x.decode("utf8") for x in p.stdout.readlines()]:
        ...
        if major == 1 and minor == 19 and micro < 2:
            continue
        # find last commit before tag in module subdirectory
        cmd = ["git", "log", "--pretty=format:%H", "-1", version_str, "--", "."]
        r = subprocess.run(
            cmd, capture_output=True, text=True, check=True, cwd=meson_source_root
        )
        commit_hash = r.stdout.strip()
...
if __name__ == "__main__":
    ...
    with open(output_fn, "w") as f:
        ...
        if start_tag and start_tag != "start":
            ...
            output_commits(module, "monorepo-start", "HEAD", f"subprojects/{module}/")
            ...
            output_commits(module, module_start, f"{module_tag_prefix}1.19.2", None)
