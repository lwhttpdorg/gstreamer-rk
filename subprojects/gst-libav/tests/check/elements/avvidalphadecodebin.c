/* GStreamer unit tests for libav alpha decode bins
 *
    ...
    const gchar * expected_sink_caps)
{
  ...
  fail_unless_equals_int (gst_plugin_feature_get_rank (GST_PLUGIN_FEATURE
          (factory)), GST_RANK_MARGINAL + GST_ALPHA_DECODE_BIN_RANK_OFFSET);
...
GST_START_TEST (test_avdec_vp8alphadecodebin_factory)
{
  const gchar *required[] = { "avdec_vp8", "avdec_vp8alphadecodebin" };

  if (!require_elements_or_skip (required, G_N_ELEMENTS (required)))
    return;

  check_alpha_decode_bin_factory ("avdec_vp8alphadecodebin",
      "video/x-vp8, codec-alpha = (boolean) true");
}

GST_END_TEST;

GST_START_TEST (test_avdec_vp9alphadecodebin_factory)
{
  const gchar *required[] = { "avdec_vp9", "avdec_vp9alphadecodebin" };

  if (!require_elements_or_skip (required, G_N_ELEMENTS (required)))
    return;

  check_alpha_decode_bin_factory ("avdec_vp9alphadecodebin",
...
GST_START_TEST (test_avdec_vp8alphadecodebin_smoke)
{
  ...
  const gchar *required[] = { "vp8enc", "avdec_vp8", "codecalphademux",
    "alphacombine", "avdec_vp8alphadecodebin"
  };

  if (!require_elements_or_skip (required, G_N_ELEMENTS (required)))
    return;

  run_alpha_decode_bin_smoke ("avdec_vp8alphadecodebin",
      "video/x-vp8,codec-alpha=(boolean)true", launchline);
}

GST_END_TEST;

GST_START_TEST (test_avdec_vp9alphadecodebin_smoke)
{
  const gchar *launchline =
      "videotestsrc num-buffers=1 pattern=smpte ! "
      "video/x-raw,format=I420,width=16,height=16,framerate=1/1 ! "
  ...
  const gchar *required[] = { "vp9enc", "avdec_vp9", "vp9parse",
    "codecalphademux", "alphacombine", "avdec_vp9alphadecodebin"
  };

  if (!require_elements_or_skip (required, G_N_ELEMENTS (required)))
    return;

  run_alpha_decode_bin_smoke ("avdec_vp9alphadecodebin",
...
static Suite *
avvidalphadecodebin_suite (void)
{
  Suite *s = suite_create ("avvidalphadecodebin");
  TCase *tc_chain = tcase_create ("general");

  tcase_set_timeout (tc_chain, 20);
  suite_add_tcase (s, tc_chain);

  tcase_add_test (tc_chain, test_avdec_vp8alphadecodebin_factory);
  tcase_add_test (tc_chain, test_avdec_vp9alphadecodebin_factory);
  tcase_add_test (tc_chain, test_avdec_vp8alphadecodebin_smoke);
  tcase_add_test (tc_chain, test_avdec_vp9alphadecodebin_smoke);

  return s;
}

GST_CHECK_MAIN (avvidalphadecodebin);
