/* GStreamer unit tests for avparse_g729
 *
 * Copyright (C) 2024 Devin Anderson <danderson@microsoft.com>
 *
...
#define SIMPLE_G729_PATH MAKE_RESOURCE_PATH("sine.g729")
...
create_caps_change_base_pipeline (GstElement ** concat, GstElement ** parser)
{
  ...
  *parser = gst_element_factory_make ("avparse_g729", "avparse_g729");
  fail_if (*parser == NULL);
...
{
  GstCaps *caps = gst_caps_new_simple ("audio/G729",
...
GST_START_TEST (test_bad_caps_pull)
{
  ...
  setup_bad_caps_pipeline (pipeline, SIMPLE_G729_PATH, &buffer);
...
GST_START_TEST (test_caps_change_pull)
{
  ...
  setup_caps_change_pipeline (pipeline, SIMPLE_G729_PATH, SIMPLE_G729_PATH,
      &buffer_1, &buffer_2);
...
GST_START_TEST (test_simple_file_pull)
{
  GstBuffer *buffer;
  GstElement *pipeline = create_simple_pull_pipeline ();
  setup_simple_pipeline (pipeline, SIMPLE_G729_PATH, &buffer);
  run_success_test (pipeline);
  gst_object_unref (pipeline);
}
...
static Suite *
avparse_g729_suite (void)
{
  Suite *s = suite_create ("avparse_g729");
