...
class BinSubclassTest(TestCase):
    ...
    def testStateChange(self):
        bin = MyBin("mybin")
        self.assertEqual(bin.__gstrefcount__, 1)
        self.assertEqual(sys.getrefcount(bin), pygobject_2_13 and 2 or 3)

        self.assertEqual(bin.get_name(), "mybin")
        self.assertEqual(bin.__gstrefcount__, 1)

        # test get_state with no timeout
        (ret, state, pending) = bin.get_state()
        self.assertNotEqual(ret, gst.STATE_CHANGE_FAILURE)
        self.assertEqual(bin.__gstrefcount__, 1)

        # set to playing
        bin.set_state(gst.STATE_PLAYING)
        self.assertTrue(bin._state_changed)
        ...
        if ret == gst.STATE_CHANGE_SUCCESS:
            self.assertEqual(state, gst.STATE_PLAYING)
            self.assertEqual(pending, gst.STATE_VOID_PENDING)
...
class Preroll(TestCase):
    ...
    def tearDown(self):
        ...
        while self.bin.__gstrefcount__ > 1:
            time.sleep(0.1)
        self.assertEqual(self.bin.__gstrefcount__, 1)
        del self.bin
        TestCase.tearDown(self)
    ...
    def testFake(self):
        ...
        self.assertEqual(ret[0], gst.STATE_CHANGE_SUCCESS)
        self.assertEqual(ret[1], gst.STATE_PAUSED)
        self.assertEqual(ret[2], gst.STATE_VOID_PENDING)
        ...
        self.assertEqual(ret[0], gst.STATE_CHANGE_ASYNC)
        self.assertEqual(ret[1], gst.STATE_PAUSED)
        self.assertEqual(ret[2], gst.STATE_PAUSED)
