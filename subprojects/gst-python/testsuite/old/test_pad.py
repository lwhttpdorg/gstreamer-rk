...
class PadTemplateTest(TestCase):
    def testConstructor(self):
        ...
        self.assertEqual(sys.getrefcount(template), pygobject_2_13 and 2 or 3)
...
class PadPushUnlinkedTest(TestCase):
    ...
    def testNoProbe(self):
        ...
        self.assertEqual(self.buffer.__grefcount__, 1)
        self.assertEqual(self.src.push(self.buffer), gst.FLOW_NOT_LINKED)
    ...
    def testFalseProbe(self):
        ...
        self.buffer = gst.Buffer()
        self.assertEqual(self.buffer.__grefcount__, 1)
        self.assertEqual(self.src.push(self.buffer), gst.FLOW_OK)
        self.assertEqual(self.buffer.__grefcount__, 1)
...
class PadPushLinkedTest(TestCase):
    ...
    def testNoProbe(self):
        ...
        self.assertEqual(self.buffer.__grefcount__, 2)
    ...
    def testTrueProbe(self):
        ...
        self.assertFalse(self.buffer is self.buffers[0])
        self.assertFalse(id(self.buffer) == id(self.buffers[0]))
        # ... but they wrap the same GstBuffer
        self.assertTrue(self.buffer == self.buffers[0])
        self.assertEqual(repr(self.buffer), repr(self.buffers[0]))
...
class PadPushEventLinkedTest(TestCase):
    ...
    def testNoProbe(self):
        ...
        self.assertEqual(self.event.__grefcount__, 1)
        gst.debug('pushing event on linked pad, no probe')
        self.assertEqual(self.src.push_event(self.event), True)
        ...
        self.assertEqual(self.src.__grefcount__, 2)
        # clear it
        self.event = None
        self.assertEqual(self.src.__grefcount__, 1)
    ...
    def testFalseProbe(self):
        ...
        self.assertEqual(self.src.push_event(self.event), False)
        # one ref in our local scope, another in self.events
        self.assertEqual(self.event.__grefcount__, 2)
        self.assertEqual(self.sink.__grefcount__, 1)
        # the event has reffed the src pad as the src of the event
        self.assertEqual(self.src.__grefcount__, 2)
    ...
    def testTrueProbe(self):
        ...
        # they are not the same Python object ...
        self.assertFalse(self.event is self.events[0])
        self.assertFalse(id(self.event) == id(self.events[0]))
        # ... but they wrap the same GstEvent
        self.assertEqual(repr(self.event), repr(self.events[0]))
        self.assertTrue(self.event == self.events[0])
...
class PadTest(TestCase):
    def testConstructor(self):
        ...
        self.assertTrue(pad)
        self.assertEqual(sys.getrefcount(pad), pygobject_2_13 and 2 or 3)
        self.assertEqual(pad.__gstrefcount__, 1)

        gst.debug('creating pad with no name')
        self.assertTrue(gst.Pad(None, gst.PAD_SRC))
        self.assertTrue(gst.Pad(name=None, direction=gst.PAD_SRC))
        self.assertTrue(gst.Pad(direction=gst.PAD_SRC, name=None))
        self.assertTrue(gst.Pad(direction=gst.PAD_SRC, name="src"))
...
class PadProbePipeTest(TestCase):
    def setUp(self):
        ...
        self.assertEqual(self.fakesrc.__gstrefcount__, 1)
        self.assertEqual(sys.getrefcount(self.fakesrc), pygobject_2_13 and 2 or 3)

        self.pipeline.add(self.fakesrc, self.fakesink)
        self.assertEqual(self.fakesrc.__gstrefcount__, 2) # added
        self.assertEqual(sys.getrefcount(self.fakesrc), pygobject_2_13 and 2 or 3)
        self.assertEqual(self.fakesink.__gstrefcount__, 2) # added
        self.assertEqual(sys.getrefcount(self.fakesink), pygobject_2_13 and 2 or 3)

        self.fakesrc.link(self.fakesink)

        self.assertEqual(self.pipeline.__gstrefcount__, 1)
        self.assertEqual(sys.getrefcount(self.pipeline), pygobject_2_13 and 2 or 3)
        self.assertEqual(self.fakesrc.__gstrefcount__, 2)
        self.assertEqual(sys.getrefcount(self.fakesrc), pygobject_2_13 and 2 or 3)
        self.assertEqual(self.fakesink.__gstrefcount__, 2)
        self.assertEqual(sys.getrefcount(self.fakesink), pygobject_2_13 and 2 or 3)
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.fakesrc.__gstrefcount__, 1) # parent gone
        self.assertEqual(self.fakesink.__gstrefcount__, 1) # parent gone
        self.assertEqual(sys.getrefcount(self.fakesrc), pygobject_2_13 and 2 or 3)
        self.assertEqual(sys.getrefcount(self.fakesink), pygobject_2_13 and 2 or 3)
    ...
    def testFakeSrcProbeOnceKeep(self):
        ...
        self.assertEqual(self._got_fakesink_buffer, 1)
    ...
    def _probe_callback_fakesrc(self, pad, buffer):
        self.assertTrue(isinstance(pad, gst.Pad))
        self.assertTrue(isinstance(buffer, gst.Buffer))
...
class PadRefCountTest(TestCase):
    def testAddPad(self):
        ...
        self.assertEqual(sys.getrefcount(e), pygobject_2_13 and 2 or 3)
        self.assertEqual(e.__gstrefcount__, 1)
        ...
        self.assertEqual(e.__gstrefcount__, 1)
        self.assertEqual(sys.getrefcount(pad), pygobject_2_13 and 2 or 3)
        self.assertEqual(pad.__gstrefcount__, 2) # added to element
        ...
        if not pygobject_2_13:
            ...
            # pygobject >= 2.13.0
            self.assertEqual(self.gccollect(), 1) # collected the element
        self.assertEqual(sys.getrefcount(pad), pygobject_2_13 and 2 or 3)
        self.assertEqual(pad.__gstrefcount__, 1) # removed from element
        ...
        if not pygobject_2_13:
            ...
            # pygobject >= 2.13.0
            self.assertEqual(self.gccollect(), 1) # collected the pad
...
class PadBlockTest(TestCase):
    ...
    def testCallbackRefcount(self):
        ...
        self.assertEqual(cb_refcount, 2)
       
        pad = gst.Pad('src', gst.PAD_SRC)
        pad.set_active(True)
        pad.set_blocked_async(True, blocked_cb)
        # set_blocked_async refs the callback
        self.assertEqual(sys.getrefcount(blocked_cb), 3)
        ...
        self.assertEqual(sys.getrefcount(blocked_cb), cb_refcount)
