...
ci-fairy s3cp --token-file "${S3_JWT_FILE}" /lava-files/${KERNEL_IMAGE_NAME} \
	"${KERNEL_IMAGE_BASE}/${KERNEL_IMAGE_NAME}"

ci-fairy s3cp --token-file "${S3_JWT_FILE}" /lava-files/modules.tar.zstd \
...
for dtb in ${DEVICE_TREES}; do
	...
	if [ ! -z "${dtb_file}" ]; then
		ci-fairy s3cp --token-file "${S3_JWT_FILE}" \
		      ${dtb_file} \
