...
/* Video parser fuzzing target
 *
 * Exercises all parsers from gst-plugins-bad/gst/videoparsers/ by pushing
...
static void
fuzz_videoparser (const gchar * element_name, GstCaps * caps,
...
LLVMFuzzerTestOneInput (const guint8 * data, size_t size)
{
  ...
  fuzz_videoparser ("h264parse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_empty_simple ("video/x-h265");
  fuzz_videoparser ("h265parse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_empty_simple ("video/x-h266");
  fuzz_videoparser ("h266parse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_empty_simple ("video/x-h263");
  fuzz_videoparser ("h263parse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_simple ("video/mpeg", "mpegversion", G_TYPE_INT, 2, NULL);
  fuzz_videoparser ("mpegvideoparse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_simple ("video/mpeg", "mpegversion", G_TYPE_INT, 4, NULL);
  fuzz_videoparser ("mpeg4videoparse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_empty_simple ("video/x-av1");
  fuzz_videoparser ("av1parse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_empty_simple ("video/x-vp9");
  fuzz_videoparser ("vp9parse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_simple ("video/x-wmv", "wmvversion", G_TYPE_INT, 3, NULL);
  fuzz_videoparser ("vc1parse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_empty_simple ("video/x-dirac");
  fuzz_videoparser ("diracparse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_empty_simple ("image/png");
  fuzz_videoparser ("pngparse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_empty_simple ("image/x-jpc");
  fuzz_videoparser ("jpeg2000parse", caps, data, size);
  gst_caps_unref (caps);

  caps = gst_caps_new_empty_simple ("image/jpeg");
  fuzz_videoparser ("jpegparse", caps, data, size);
  gst_caps_unref (caps);
